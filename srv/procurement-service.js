...
module.exports = function () {
    ...
    this.before('CREATE', 'PurchaseOrders', async (req) => {
        ...
        if (supplier_ID) {
            ...
            if (!supplier) {
                req.error(404, 'Supplier not found', 'supplier_ID');
            }
 
            if (supplier && !supplier.isActive) {
                req.error(400, 'Supplier is inactive', 'supplier_ID');
            }
        }
        ...
        if (!req.data.poNumber) {
            ...
            req.data.poNumber =
                `PO-${String(count.length + 1).padStart(5, '0')}`;
        }
    ...
    this.before('UPDATE', 'PurchaseOrders', async (req) => {
        ...
        if (po && po.status !== 'Draft' && po.status !== 'Rejected') {
            ...
            const attemptedFields = Object.keys(req.data)
                .filter(f => f !== 'modifiedAt' && f !== 'modifiedBy');
 
            const blockedFields = attemptedFields
    ...
    this.before('CREATE', 'PurchaseOrderItems', async (req) => {
        ...
        if (!product_ID)
            req.error(400, 'Product is required', 'product_ID');
        ...
        if (product_ID) {
            ...
            if (!product)
                req.error(404, 'Product not found', 'product_ID');
        }
    ...
    this.on('submit', 'PurchaseOrders', async (req) => {
        ...
        if (po.status !== 'Draft' && po.status !== 'Rejected') {
            req.reject(
                400,
                `Cannot submit: PO is "${po.status}". Only Draft or Rejected POs can be submitted.`
            );
        }
        ...
        if (items.length === 0) {
            req.reject(
                400,
                'Cannot submit: PO has no line items. Add at least one item.'
            );
        }
    ...
    this.on('approve', 'PurchaseOrders', async (req) => {
        ...
        if (po.status !== 'Pending') {
            req.reject(
                400,
                `Cannot approve: PO is "${po.status}". Only Pending POs can be approved.`
            );
        }
...
this.on('reject', 'PurchaseOrders', async (req) => {
    ...
    if (po.status !== 'Pending') {
        req.reject(
            400,
            `Cannot reject: PO is "${po.status}". Only Pending POs can be rejected.`
        );
    }
...
this.on('resubmit', 'PurchaseOrders', async (req) => {
    ...
    if (po.status !== 'Rejected') {
        req.reject(
            400,
            `Cannot resubmit: PO is "${po.status}". Only Rejected POs can be resubmitted.`
        );
    }
...
this.on('markOrdered', 'PurchaseOrders', async (req) => {
    ...
    if (po.status !== 'Approved') {
        req.reject(
            400,
            `Cannot mark as ordered: PO must be Approved. Current: "${po.status}"`
        );
    }
...
this.on('receiveGoods', 'PurchaseOrders', async (req) => {
    ...
    if (po.status !== 'Ordered') {
        req.reject(
            400,
            `Cannot receive: PO must be "Ordered". Current: "${po.status}"`
        );
    }
...
this.on('getDashboard', async () => {
    ...
    const byStatus = (status) =>
        allPOs.filter(p => p.status === status).length;
...
this.on('POSubmitted', (msg) => {
    ...
    console.log(
        `${icon} [SUBMITTED] ${poNumber} | $${amount} | Priority: ${priority}`
    );
});
...
this.on('POApproved', (msg) => {
    ...
    console.log(
        `✅ [APPROVED] ${msg.data.poNumber} by ${msg.data.approver}`
    );
});
...
this.on('PORejected', (msg) => {
    ...
    console.log(
        `❌ [REJECTED] ${msg.data.poNumber} | Reason: ${msg.data.reason}`
    );
});
...
this.on('StockUpdated', (msg) => {
    ...
    console.log(
        `📦 [STOCK] ${productName}: ${oldStock} → ${newStock} (+${newStock - oldStock})`
    );
});
